...
func TestValidateWithProgress(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(tempDir, "calls"), 0o750)
	_ = os.MkdirAll(filepath.Join(tempDir, "sms"), 0o750)
	_ = os.MkdirAll(filepath.Join(tempDir, "attachments"), 0o750)
...
created_by: "test"
`
	err := os.WriteFile(filepath.Join(tempDir, ".mobilecombackup.yaml"), []byte(markerContent), 0o600)
	if err != nil {
