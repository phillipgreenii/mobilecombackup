...
// msgWorkspaceAlreadyClean is the status message used whenever cleanup
...
func (cp *CompletionProtocol) CommitChanges(issueID, taskDescription string) error {
	...
	filesToStage := make([]string, 0, len(ws.ModifiedFiles)+len(ws.UntrackedFiles))
...
func (cp *CompletionProtocol) EnsureCleanCompletion(issueID, taskDescription string) *CompletionResult {
	...
	if ws.IsClean {
		...
		result.Message = msgWorkspaceAlreadyClean
...
func (wc *WorkspaceCleanup) CleanupWorkspace() (*CompletionResult, error) {
	...
	if ews.IsClean && ews.GitState.IsClean {
		return &CompletionResult{
			Status:    StatusComplete,
			Message:   msgWorkspaceAlreadyClean,
...
func (wc *WorkspaceCleanup) commitWithMessage(message string) error {
	// message is generated by generateCommitMessage, not external input; and
	// exec.Command never invokes a shell, so there is no injection surface.
